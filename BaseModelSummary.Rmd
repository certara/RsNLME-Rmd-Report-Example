...
htmltools::img(src = knitr::image_uri(file.path('img', 'certara.PNG')), 
               alt = 'logo', 
               style = 'position:fixed; top:0; right:0; padding:5px; width:300px;')
```
...
```{r base-vpc, fig.retina=2, out.width="140%"}
...
vpcPlot <- ggplot(vpc$stats, aes(x = x)) +
  ...
  geom_point(data = vpc$obs[!(blq | alq)], aes(x = x, y = y), color = "#757D8F", size = 1.5, shape = 16, alpha = 0.8, show.legend = FALSE) +
...
*  99+% target achievement for Cmax; 69% for WT > 90 Kg
*  99+% target achievement for AUC24; 30% for WT > 90 Kg
*  Consider dose adjustment for > 90 Kg subjects
:::
...
p1<-tag_facet(p1, x=Inf,y=Inf,                        
          ...
          tag_pool=c(paste("%OverTarget = ",round(overtargetAUC24,0)),   #this uses the % values we created earlier
                     paste("%OverTarget = ",round(overtargetCmax,0))),
          hjust = 1.1,
          vjust = 1.5) +
...
## Facet by WT Categories <50kg, 50-80kg, >90kg
TAplotdat <- TAplotdat %>% 
  mutate(WTcut = cut(WT,breaks=c(30,50,90,Inf)))

#calculate AUC24 and Cmax attainment by cuts
